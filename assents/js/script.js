...
export function carregarTabela() {
    ...
    onValue(refDB, (snapshot) => {
        ...
        numerosOrdenados.forEach(num => {
            ...
            tabela.innerHTML += `
                <tr>
                    <td class="idCl">${cliente.numero}</td>
                    <td>${cliente.nome}</td>
                    <td class="alteracao"> 
                        <i class="bi bi-pencil-square" title="editar" 
                        onclick="abrirEditor(${cliente.numero}, '${cliente.nome}', ${cliente.telefone})"></i>

                        <i class="bi bi-trash" title="deletar" onclick="excluirCliente(${cliente.numero})"></i>
                    </td>
                </tr>`;
        });
        inscritos.innerHTML = `Cadastrados: ${todosClientes.length}`;
    });
}
...
let clienteId = null;
let name = document.querySelector('#novoNome');
let number  = document.querySelector('#novoNumero');

window.abrirEditor = function(id, nome, numero){
